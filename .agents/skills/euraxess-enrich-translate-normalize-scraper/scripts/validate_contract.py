#!/usr/bin/env python3
"""Validate the closed EURAXESS nomad-agent-job-v1 wire contract."""
...
def _hiring_contact(value: Any, path: str) -> None:
    ...
    for field in keys - {"address", "name"}:
        _string(item[field], f"{path}.{field}", nullable=True)
...
def _unmapped_information(value: Any, path: str) -> None:
    if value is None:
        return
    if not isinstance(value, Mapping):
        raise ContractValidationError(f"{path} must be an object or null")
    for key, entries in value.items():
        _string(key, f"{path} key")
        _list(entries, f"{path}.{key}", _string_item)


def _unparsed_geofield(value: Any, path: str) -> None:
    item = _object(value, path, {"locationIndex", "raw"})
    _number(
        item["locationIndex"],
        f"{path}.locationIndex",
        integer=True,
        minimum=0,
    )
    _string(item["raw"], f"{path}.raw")
...
def _euraxess_custom(value: Any, path: str) -> None:
    ...
    data = _object(
        ...
        {
            "academicLevelRaw",
            "researchInfrastructureStaffPosition",
            "unmappedJobInformation",
            "unparsedGeofields",
        },
    )
    academic = _list(
        data["academicLevelRaw"],
        f"{path}.data.academicLevelRaw",
        _string_item,
        nullable=True,
    )
    if academic is not None and len(academic) != len(set(academic)):
        raise ContractValidationError(
            f"{path}.data.academicLevelRaw must contain unique values"
        )
    _string(
        data["researchInfrastructureStaffPosition"],
        f"{path}.data.researchInfrastructureStaffPosition",
        nullable=True,
    )
    _unmapped_information(
        data["unmappedJobInformation"],
        f"{path}.data.unmappedJobInformation",
    )
    _list(
        data["unparsedGeofields"],
        f"{path}.data.unparsedGeofields",
        _unparsed_geofield,
        nullable=True,
    )


def validate_euraxess_job(value: Any) -> Mapping[str, Any]:
    ...
    if identity["source"] != "euraxess":
        raise ContractValidationError("identity.source must be euraxess")
    ...
    _euraxess_custom(record["custom"], "custom")
