#!/usr/bin/env python3
"""Validate EURAXESS nomad-agent-job-v1 output without losing the source record."""
...
from validate_contract import ContractValidationError, validate_euraxess_job
...
def validate_normalized_job(item: Any) -> Mapping[str, Any]:
    try:
        return validate_euraxess_job(item)
    except ContractValidationError as exc:
        raise OutputContractError(str(exc)) from exc


def parse_euraxess_output(item: Any) -> dict[str, Any]:
    ...
    custom = _mapping(record["custom"], "custom")
    custom_data = _mapping(custom.get("data"), "custom.data")
    ...
    return {
        ...
        "academicLevelRaw": custom_data.get("academicLevelRaw"),
...
def parse_euraxess_outputs(items: Iterable[Any]) -> list[dict[str, Any]]:
    return [parse_euraxess_output(item) for item in items]
...
def main() -> int:
    ...
    parsed = parse_euraxess_outputs(load_records(args.input))
