...
exact build `1.0.16`. Verify Actor availability and the completed run's build
before accepting results.

Use Apify MCP through generic `call-actor` with
`callOptions.build: "1.0.16"`; do not rely on a mutable tag. Keep the nested
...
2. Use only generic `call-actor` with exact
   `callOptions.build: "1.0.16"`. Require the run response to report
   `buildNumber: "1.0.16"` before accepting its output.
...
```json
{
  ...
  "waitSecs": 0,
  "callOptions": {"build": "1.0.16", "maxItems": 5, "maxTotalChargeUsd": 0.1}
}
```

1. Call generic `call-actor`; do not rely on the direct Actor tool or a mutable
   tag.
2. Require the authoritative run to report `buildNumber: "1.0.16"`. If MCP
