...
These are bounded contract examples for selector `latest`. Every MCP
execution must use generic `call-actor` with `callOptions.build: "latest"` and
verify that exact authoritative run build rather than rely on a mutable tag.

## Focused research search

```text
After confirming Actor access and current pricing, call the `latest` selector
