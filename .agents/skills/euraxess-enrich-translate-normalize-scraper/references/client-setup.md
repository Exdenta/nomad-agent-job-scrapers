...
Use generic
`call-actor` with exact `callOptions.build: "1.0.13"` and verify the same build
...
3. Call generic `call-actor` with the canonical Actor name, strict v1 input,
   `callOptions.build: "1.0.13"`, `callOptions.maxItems: 5`, and a conservative
   `callOptions.maxTotalChargeUsd`.
4. Require the run response to report `buildNumber: "1.0.13"` and stop on any
   mismatch.
