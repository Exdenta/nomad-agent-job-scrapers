# Minimal public RUN-SUMMARY v4

A completed usable Actor run writes `RUN-SUMMARY` in its default key-value
store with discriminator `nomad-agent-run-summary-v4`.
...
- atomic `retry`: `recommended` and `afterSeconds`

`succeeded` and `partial` require at least one delivered job. `empty`
requires zero delivered jobs, `resultsLimited: false`, and no retry. A recommended retry
