...
class EuraxessLatestTests(unittest.TestCase):
    ...
    def test_n8n_rejects_malformed_build_id_even_when_receipts_agree(self):
        workflow = json.loads((ROOT / 'integrations/n8n/euraxess-jobs-to-google-sheets.json').read_text())
        code = next(n['parameters']['jsCode'] for n in workflow['nodes'] if n['name'] == 'Validate terminal run')
        runner = "global.$runIndex=0; const p=JSON.parse(require('fs').readFileSync(0,'utf8')); const fn=new Function('$input','$',p.code); process.stdout.write(JSON.stringify(fn({first:()=>({json:p.run})},name=>({first:()=>({json:name==='Configuration'?{actorBuild:'latest'}:p.run})}))));"
        receipt = {'id': 'run', 'status': 'SUCCEEDED', 'exitCode': 0, 'buildNumber': '1.0.999', 'defaultDatasetId': 'dataset'}
        for build_id in ['futureImmutableBuild', '   ', 42, {}, []]:
            with self.subTest(build_id=build_id):
                result = subprocess.run(['node', '-e', runner], input=json.dumps({'code': code, 'run': dict(receipt, buildId=build_id)}), text=True, capture_output=True)
                if build_id == 'futureImmutableBuild':
                    self.assertEqual(result.returncode, 0, result.stderr)
                else:
                    self.assertNotEqual(result.returncode, 0)
                    self.assertIn('omitted its resolved build identity', result.stderr)
