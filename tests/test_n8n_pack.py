...
global.$ = (name) => {
  if (name === 'Configuration') return { first: () => ({ json: { actorBuild: '0.6.45' } }) };
...
"""
    ...
    def test_apify_request_uses_header_auth_and_bounded_v1_input(self) -> None:
        ...
        self.assertEqual(assignments["actorBuild"], "0.6.45")
    ...
    def test_failed_or_wrong_build_run_is_not_retried_or_delivered(self) -> None:
        failed = self.run_terminal_node({
            ...
            "buildNumber": "0.6.45", "defaultDatasetId": "dataset-2",
        })
        ...
        self.assertIn("expected 0.6.45", wrong_build.stderr)
