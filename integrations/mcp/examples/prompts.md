...
run to SUCCEEDED and retrieve the default dataset with get-dataset-items.
...
I approve a changed input. For an error, report runId, status, statusMessage,
and exitCode and tell me to inspect the Apify run log; never present partial
dataset rows as a successful search.
```
