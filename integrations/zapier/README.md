...
5. Filter each row: schema is `nomad-ai-job-fit-v1`, scoring algorithm is
   `scoring-v3`, and status is not `ai_failed`.
...
Before enabling the Zap, inspect `RUN-SUMMARY` v3 manually or add an exact
key-value-store Webhook step. Reconcile the single `job-fit-result` meter at
$0.02 per successful row, confirm the $0.25/two-attempt owner-provider circuit
