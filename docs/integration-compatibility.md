...
| --- | --- | --- | --- |
| LinkedIn | `0.6.40` (`t9hg45cUQtARUP6kK`) | Public `latest` and `canary` point to `0.6.40` | `schemaVersion`, `keyword`, `location`, `linkedinSearch`, `strictGeography`, `workArrangements`, `postedWithin`, `filters`, `companyProfileEnrichment`, `companyFilters`, `maxItems`, `translateToEnglish`, `aiEnrichment`, `includeRaw`, `dedupe`, `analyticsEnabled` |
...
| --- | --- | --- | --- | --- |
| n8n | Workflow pins `0.6.40` | Workflow pins `1.0.9` | Yes. Simple safe fields plus `advancedInputJson`, merged and validated before the paid call | Validates v3, retries once when valid, then reconciles and projects the dataset |
| Make | Apify Task must pin `0.6.40` | Apify Task must pin `1.0.9` | Yes. The Task owns the complete Actor input; the blueprint consumes its completed run | Reads v3 and can repeat the same Task once before projecting rows |
| MCP | Generic `call-actor` pins `callOptions.build: "0.6.40"` | Generic `call-actor` pins `callOptions.build: "1.0.9"` | Yes. Actor input is passed without a reduced integration schema | REST-verified terminal run; validated v3; one retry; reconciled canonical rows |
| REST API and webhooks | `build=0.6.40` | `build=1.0.9` | Yes. JSON request body is the Actor input | Poll/read the exact run, factual status, and dataset; a webhook is only a completion signal |
...
`2025-06-18`: LinkedIn run `fGUBRCL6PmDZbOzGr` returned five reconciled
...
current `0.6.40` n8n/Make destination paths and all EURAXESS destinations still
...
hosted-MCP release smokes passed for LinkedIn `0.6.40` and EURAXESS `1.0.9` on
