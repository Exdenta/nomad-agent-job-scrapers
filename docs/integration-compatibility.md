...
| --- | --- | --- | --- |
| LinkedIn | `0.6.41` (`WJVinrdAxORkaLF3l`) | Public `latest` and `canary` point to `0.6.41` | `schemaVersion`, `keyword`, `location`, `linkedinSearch`, `strictGeography`, `workArrangements`, `postedWithin`, `filters`, `companyProfileEnrichment`, `companyFilters`, `maxItems`, `translateToEnglish`, `aiEnrichment`, `includeRaw`, `dedupe`, `analyticsEnabled` |
| EURAXESS | `1.0.10` (`LjfHtUT5TbAdffWBL`) | Private `latest` and `canary` point to `1.0.10` | `schemaVersion`, `keyword`, `location`, `euraxessSearch`, `workArrangements`, `postedWithin`, `filters`, `maxItems`, `translateToEnglish`, `aiEnrichment`, `includeRaw`, `dedupe`, `analyticsEnabled` |

The two Actors share the six-root `nomad-agent-job-v1` dataset-row envelope and
the same minimal `nomad-agent-run-summary-v4` delivery and one-retry gate. Their source
...
| --- | --- | --- | --- | --- |
| n8n | Workflow pins `0.6.41` | Workflow pins `1.0.10` | Yes. Simple safe fields plus `advancedInputJson`, merged and validated before the paid call | Validates v4, retries once when valid, then reconciles and projects the dataset |
| Make | Apify Task must pin `0.6.41` | Apify Task must pin `1.0.10` | Yes. The Task owns the complete Actor input; the blueprint consumes its completed run | Reads v4 and can repeat the same Task once before projecting rows |
| MCP | Generic `call-actor` pins `callOptions.build: "0.6.41"` | Generic `call-actor` pins `callOptions.build: "1.0.10"` | Yes. Actor input is passed without a reduced integration schema | REST-verified terminal run; validated v4; one retry; reconciled canonical rows |
| REST API and webhooks | `build=0.6.41` | `build=1.0.10` | Yes. JSON request body is the Actor input | Poll/read the exact run, factual status, and dataset; a webhook is only a completion signal |
...
current input-key pass-through, terminal-run and v4 status gating, canonical
output, flat projection, secret hygiene, and the hard one-retry bound. Apify metadata confirms the
build/tag and input-schema facts above. Actor/API canaries passed on 2026-08-13
for both exact builds: LinkedIn run `3Q4yWRpCnbU8iYSVk` validated v4 and returned
one reconciled canonical row; EURAXESS run `Fgo5aehGjDm3Q7GQF` validated v4 and
returned a reconciled valid empty result. Prior authenticated hosted-MCP
end-to-end smokes used LinkedIn `0.6.40` and EURAXESS `1.0.9` with v3.
LinkedIn's MCP-origin compatibility layer also
...
current `0.6.41` n8n/Make destination paths and all EURAXESS destinations still
need credentialed live smoke tests. The MCP smoke harness requires an
`APIFY_TOKEN`, but the checked-in scripts contain no credential. Actor/API
release canaries passed for LinkedIn `0.6.41` and EURAXESS `1.0.10` on
